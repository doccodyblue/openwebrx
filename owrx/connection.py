...
class OpenWebRxClient(Client, metaclass=ABCMeta):
    ...
    def _is_timeout_exempt(self):
        ...
        return ClientRegistry.getSharedInstance().isTimeoutExempt(self.conn.handler)
...
class OpenWebRxReceiverClient(OpenWebRxClient, SdrSourceEventClient):
    ...
    def setProfile(self, sdr: str, profile: str, key: str = None):
        ...
        magic = self.stack["magic_key"]
        if self.sdr.isProfileDisabled(profile):
            # Disabled profiles cannot be selected, not even with a key
            self.write_log_message("This profile is disabled, keeping current profile.")
            # Force update back to the current profile
            self.resetSdr()

        elif self.sdr.isLocked(profile) and magic != "" and key != magic:
