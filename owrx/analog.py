...
class Ssb(BaseDemodulatorChain):
    def __init__(self, agcProfile: AgcProfile = AgcProfile.FAST):
        agc = Agc(Format.FLOAT)
        agc.setProfile(agcProfile)
        agc.setInitialGain(200)  # Prevent AGC clamping on mode switch transient
        agc.setMaxGain(1500)      # Allow enough gain for weak signals
        agc.setReference(0.5)    # Target level with some headroom
        workers = [
            RealPart(),
            Limit(),             # Soft-clip before AGC to prevent harsh distortion
            agc,
        ]
        super().__init__(workers)
