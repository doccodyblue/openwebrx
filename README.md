...
* **Disabled Profiles** - Profiles with `disabled=true` are hidden from all users, magic key or not. Configurable via checkbox in Admin Interface under profile settings. Useful for temporarily taking profiles out of service (e.g., when the connected antenna/amplifier does not cover their frequency range) without deleting them. A currently active profile keeps running until the next profile switch.
